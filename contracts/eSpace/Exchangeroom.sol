...
import "../VotePowerQueueEspace.sol";
...
contract Exchangeroom is Ownable,Initializable {
  ...
  using VotePowerQueueEspace for VotePowerQueueEspace.InOutQueue;
  ...
  VotePowerQueueEspace.InOutQueue private Inqueues;
  mapping(address => VotePowerQueueEspace.InOutQueue) private userOutqueues;
  // ======================== Modifiers =================================
  ...
  function XCFX_burn(uint256 _amount) public virtual onlyRegisted returns(uint256, uint256){
    ...
    if(_mode == 1){
      userOutqueues[msg.sender].enqueue(VotePowerQueueEspace.QueueNode(_amount, cfx_back, block.number + _poolLockPeriod_fast));
      speedMode = 101109; //fast code
      }
    else{
      userOutqueues[msg.sender].enqueue(VotePowerQueueEspace.QueueNode(_amount, cfx_back, block.number + _poolLockPeriod_slow));
  ...
  function userOutQueue(address account) public view returns (VotePowerQueueEspace.QueueNode[] memory) {
